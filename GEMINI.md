...
# pact protocol — seat `gemini-worker`

This repo uses the **pact protocol** (v1). You are seat `gemini-worker`, roles: worker.
...
```bash
export PACT_AGENT_ID=gemini-worker
pactify join gemini-worker --roles worker
```

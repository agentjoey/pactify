...
- **P3-8 worker pact 钉到 driver worktree(`PACT_DIR` pin)✅(2026-06-24)**:**opencode 投递丢失现场**——`.pact` gitignored ⇒ ledger 每 worktree 一份、不经 git 共享;driver 把 worker 隔离进独立 worktree,但 worker 的 checkpoint(经 cwd 绑定的 MCP)靠 agent 自己解析 cwd/项目根去定位 `.pact`+git,opencode 的项目级 MCP 独立解析 → 落到**与 driver 岔开的 ledger/分支**,worktree 销毁即丢,driver 报「failure limit / no checkpoint evidence」。修复:① runner 给 worker 注**绝对 `PACT_DIR=<RepoDir>/.pact`**,把 worker 的 pact 钉到 driver 的 worktree;② `pact.At(".")` 当 `PACT_DIR` 为绝对路径时,git 工作目录取 `filepath.Dir(PACT_DIR)`(此前绝对 `PACT_DIR` 只钉 ledger 路径、git 仍在 cwd——「ledger 与 git 分家」正是 bug 的一半)。测试 `TestCheckpointHonorsAbsolutePactDir`(外来 cwd + 绝对 PACT_DIR 仍提交到正确仓)、`TestRunnerStampsTaskAndProjectEnv`(注 `PACT_DIR`)。
- **P3-9 回灌 merge + escalation 归因 ✅(2026-06-24)**:补齐 P3-8 的两条后续。① `writeLedger`(回灌)从整文件覆盖改为**按 `event_id` union 合并** main 与 sandbox 的 log(append-only,base 在前、extra 取新),再从合并 log **重投影 STATE.yml**——seed→copy-back 之间写进 main 的并发事件不再被冲掉(`mergeEventLines`/`lineEventID`)。② `History.LastFail` 记每任务最近一次失败归因;worker 干净退出却没到 `awaiting_review` 时记「worker finished but recorded no checkpoint — no commit landed…」,crash/timeout 记「agent run failed…」,`tripped` 把它拼进 escalation reason——不再是裸 `failure limit exceeded`。测试 `TestMergeEventLines`、`TestWriteLedgerPreservesConcurrentMainEvents`、`TestEscalationNamesNoCheckpointCause`。

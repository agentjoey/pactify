...
|---------|------|---------|
| **v0.5.2** | **2026-06-17** | **SSE 反代健壮性修复：`handleEvents` 连上即 flush 初始帧（`retry: 3000` + `: ok`）+ 每 25s `: ping` 心跳 + `X-Accel-Buffering: no`。修复 dashboard 经 Cloudflare Tunnel（缓冲响应直到首字节才转发 200）时安静项目 `EventSource.onopen` 不触发、右上角恒 offline 的问题。本地工作流公网入口 pactify.agentjoey.ai 实测变 live。** |
